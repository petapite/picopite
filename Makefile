...
# Floppy image
#
floppy_image: $(BUILD_DIR)/picopite.img

$(BUILD_DIR)/picopite.img: bootloader kernel
	@dd if=/dev/zero of=$@ bs=512 count=2880 >/dev/null
	@mkfs.fat -F 12 -n "PIOS" $@ >/dev/null
...
clean:
	...
	@rm -rf src/kernel/arch/i686/isrs_gen.c src/kernel/arch/i686/isrs_gen.inc
	@rm -rf $(BUILD_DIR)/*